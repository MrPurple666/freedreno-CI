...
curl https://gitlab.freedesktop.org/PixelyIon/mesa/-/archive/kgsl-sync/mesa-kgsl-sync.zip --output mesa-main.zip &> /dev/null
###
...
meson build-android-aarch64 --cross-file $workdir/mesa-kgsl-sync/android-aarch64 -Dbuildtype=release -Dplatforms=android -Dplatform-sdk-version=31 -Dandroid-stub=true -Dgallium-drivers= -Dvulkan-drivers=freedreno -Dfreedreno-kgsl=true -Db_lto=true &> $workdir/meson_log
...
cp $workdir/mesa-kgsl-sync/build-android-aarch64/src/freedreno/vulkan/libvulkan_freedreno.so $workdir
cp $workdir/mesa-kgsl-sync/build-android-aarch64/src/android_stub/libhardware.so $workdir
cp $workdir/mesa-kgsl-sync/build-android-aarch64/src/android_stub/libsync.so $workdir
cp $workdir/mesa-kgsl-sync/build-android-aarch64/src/android_stub/libbacktrace.so $workdir
cd $workdir
